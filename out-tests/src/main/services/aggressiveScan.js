...
exports.setAggressiveScanConfig = setAggressiveScanConfig;
exports.getAggressiveScanConfig = getAggressiveScanConfig;
exports.calculateEventTier = calculateEventTier;
exports.calculateMinutesToKickoff = calculateMinutesToKickoff;
exports.createTieredEvent = createTieredEvent;
exports.isPreMatchEvent = isPreMatchEvent;
exports.upsertTieredEvent = upsertTieredEvent;
exports.getEventsForTier = getEventsForTier;
exports.getEventById = getEventById;
exports.promoteEvents = promoteEvents;
exports.getTotalEventCount = getTotalEventCount;
exports.getEventCountsByTier = getEventCountsByTier;
exports.initQuotaBudget = initQuotaBudget;
exports.calculatePollInterval = calculatePollInterval;
exports.calculateTierBudgets = calculateTierBudgets;
exports.recordRequest = recordRequest;
exports.canUseBurstBuffer = canUseBurstBuffer;
exports.isOverBudget = isOverBudget;
exports.resetHourlyQuota = resetHourlyQuota;
exports.boostEvent = boostEvent;
exports.removeBoost = removeBoost;
exports.cleanupExpiredBoosts = cleanupExpiredBoosts;
exports.isEventBoosted = isEventBoosted;
exports.getBoostedEventIds = getBoostedEventIds;
exports.updateOddsCache = updateOddsCache;
exports.getCachedOdds = getCachedOdds;
exports.evictOldEvents = evictOldEvents;
exports.getAggressiveScanStats = getAggressiveScanStats;
exports.startAggressiveScan = startAggressiveScan;
exports.stopAggressiveScan = stopAggressiveScan;
exports.isAggressiveScanRunning = isAggressiveScanRunning;
exports.initColdStart = initColdStart;
exports.updateColdStartProgress = updateColdStartProgress;
exports.getColdStartProgress = getColdStartProgress;
exports.completeColdStart = completeColdStart;
...
// State
// ============================================================================
let config = { ...types_1.DEFAULT_AGGRESSIVE_SCAN_CONFIG };
let isRunning = false;
let abortController = null;
let correlationId = null;
// Event caches
const tieredEventCache = new Map();
...
// Polling loops
const tierPollTimers = new Map();
// Cold start tracking
let coldStartProgress = null;
// ============================================================================
// Configuration
// ============================================================================
function setAggressiveScanConfig(newConfig) {
    config = { ...config, ...newConfig };
    (0, logger_1.logInfo)('aggressiveScan.config.updated', {
...
function getAggressiveScanConfig() {
    return { ...config };
}
function sanitizeConfigForLogging(cfg) {
    return {
        enabled: cfg.enabled,
 ...
 */
function calculateEventTier(event, now = Date.now()) {
    if (!event.date) {
        return 'distant';
    }
 ...
 */
function calculateMinutesToKickoff(event, now = Date.now()) {
    if (!event.date) {
        return Infinity;
    }
    const kickoffMs = new Date(event.date).getTime();
 ...
 */
function createTieredEvent(event, now = Date.now()) {
 ...
 */
function isPreMatchEvent(event, now = Date.now()) {
    if (!event.date) {
 ...
 */
function upsertTieredEvent(event, now = Date.now()) {
 ...
 */
function getEventsForTier(tier) {
 ...
 */
function promoteEvents(now = Date.now()) {
    for (const [tier, tierMap] of tieredEventCache.entries()) {
        ...
        for (const { event, newTier } of eventsToPromote) {
            ...
            (0, logger_1.logDebug)('aggressiveScan.event.promoted', {
                context: 'service:aggressiveScan',
 ...
 */
function calculatePollInterval(tier, allocatedRequests, eventCount) {
    const tierConfig = types_1.DEFAULT_TIER_CONFIGS.find(c => c.name === tier);
    if (!tierConfig) {
    ...
    // Clamp to min/max
    return Math.max(tierConfig.minPollIntervalSeconds, Math.min(tierConfig.maxPollIntervalSeconds, secondsPerPoll));
}
/**
 * Calculate tier budgets based on total budget, weights, and event counts.
 * Story 8.7 Task 3.3: Implement calculateTierBudgets
 */
function calculateTierBudgets(totalBudget, tierWeights, eventCounts) {
    const totalWeight = Object.values(tierWeights).reduce((sum, w) => sum + w, 0);
    const budgets = {
 ...
 */
function recordRequest(count = 1) {
    if (quotaBudget) {
...
function resetHourlyQuota() {
    ...
    if (quotaBudget) {
        ...
        for (const tier of Object.keys(quotaBudget.perTier)) {
 ...
 */
function boostEvent(eventId, reason = 'arb_detected') {
    // Check max concurrent boosts
    if (boostedEvents.size >= config.maxBoostedEvents) {
        // Remove oldest boost
        const oldest = Array.from(boostedEvents.values()).sort((a, b) => new Date(a.boostedAt).getTime() - new Date(b.boostedAt).getTime())[0];
        if (oldest) {
    ...
    (0, logger_1.logInfo)('aggressiveScan.event.boosted', {
        context: 'service:aggressiveScan',
 ...
 */
function cleanupExpiredBoosts(now = Date.now()) {
    const expired = [];
    ...
    for (const eventId of expired) {
        removeBoost(eventId);
        (0, logger_1.logDebug)('aggressiveScan.boost.expired', {
            context: 'service:aggressiveScan',
 ...
 */
function updateOddsCache(eventId, odds, arbsFound) {
    const event = getEventById(eventId);
    if (!event) {
        return;
    }
 ...
 */
function hasOddsChanged(oldOdds, newOdds) {
    if (!oldOdds) {
        return true;
    }
 ...
 */
function calculateVolatilityScore(cached) {
 ...
 */
function evictOldEvents(now = Date.now()) {
 ...
 */
function getAggressiveScanStats() {
 ...
 */
async function startAggressiveScan() {
    if (isRunning) {
        return;
    }
    if (!config.enabled) {
    ...
    correlationId = (0, logger_1.createCorrelationId)();
    scanStartedAt = new Date().toISOString();
    // Initialize caches
    initTierCache();
    // Initialize quota budget
    initQuotaBudget();
    (0, logger_1.logInfo)('aggressiveScan.start', {
        context: 'service:aggressiveScan',
...
function stopAggressiveScan() {
    ...
    (0, logger_1.logInfo)('aggressiveScan.stop', {
        context: 'service:aggressiveScan',
 ...
 */
function isAggressiveScanRunning() {
    return isRunning;
}
// ============================================================================
// Polling Loops
 ...
 */
function startTierPollingLoops() {
    if (!quotaBudget) {
        return;
    }
    for (const tier of Object.keys(quotaBudget.perTier)) {
...
function startTierLoop(tier) {
    ...
    const poll = async () => {
        ...
        try {
            ...
            (0, logger_1.logDebug)('aggressiveScan.poll.complete', {
                context: 'service:aggressiveScan',
                operation: 'pollTier',
                providerId: 'odds-api-io',
                correlationId: correlationId ?? undefined,
            ...
            (0, logger_1.logWarn)('aggressiveScan.poll.error', {
                context: 'service:aggressiveScan',
                operation: 'pollTier',
                providerId: 'odds-api-io',
                correlationId: correlationId ?? undefined,
    ...
    void poll();
    (0, logger_1.logInfo)('aggressiveScan.tierLoop.started', {
        context: 'service:aggressiveScan',
...
async function pollTier(tier) {
    ...
    if (isOverBudget(tier) && !canUseBurstBuffer()) {
        (0, logger_1.logDebug)('aggressiveScan.tier.overBudget', {
            context: 'service:aggressiveScan',
            operation: 'pollTier',
            providerId: 'odds-api-io',
            correlationId: correlationId ?? undefined,
 ...
 */
function initColdStart(totalEvents) {
    coldStartProgress = {
        phase: 'discovering',
        totalEvents,
        processedEvents: 0,
 ...
 */
function updateColdStartProgress(processedEvents) {
    if (!coldStartProgress) {
        return;
    }
    coldStartProgress.processedEvents = processedEvents;
    coldStartProgress.percentComplete = Math.floor((processedEvents / coldStartProgress.totalEvents) * 100);
...
exports.__test = {
    resetState() {
        stopAggressiveScan();
        config = { ...types_1.DEFAULT_AGGRESSIVE_SCAN_CONFIG };
        tieredEventCache.clear();
        oddsCache.clear();
        boostedEvents.clear();
        quotaBudget = null;

 ...
 */
function getQuoteGroupKey(quote: MarketQuote): string {
 ...
 */
function getUniqueEventMarkets(quotes: MarketQuote[]): Array<{ eventKey: string; market: string }> {
  const seen = new Set<string>()
  const result: Array<{ eventKey: string; market: string }> = []
 ...
 */
function createCrossProviderOpportunity(
  quote1: MarketQuote,
  quote2: MarketQuote,
  roi: number
): ArbitrageOpportunity {
  // Determine which quote is "home" and which is "away" based on outcome
  const [homeQuote, awayQuote] =
    quote1.outcome === 'home' || quote1.outcome === 'yes'
      ? [quote1, quote2]
      : [quote2, quote1]

  // Collect all unique provider IDs
  const providerIds = new Set<ProviderId>()
  if (homeQuote.providerId) providerIds.add(homeQuote.providerId)
  if (awayQuote.providerId) providerIds.add(awayQuote.providerId)
  const mergedFrom = Array.from(providerIds)

  // Use the oldest foundAt for staleness tracking
  const oldestFoundAt =
    homeQuote.foundAt < awayQuote.foundAt ? homeQuote.foundAt : awayQuote.foundAt
 ...
 */
export function findCrossProviderArbitrages(
  quotes: MarketQuote[]
): ArbitrageOpportunity[] {
  if (quotes.length === 0) {
    return []
  ...
  for (const { eventKey, market } of eventMarkets) {
    ...
    try {
      arbitrageOpportunitySchema.parse(opportunity)
      opportunities.push(opportunity)
    } catch (error) {
      logWarn('cross-provider.validation-failed', {
        ...
        message: `Cross-provider opportunity failed schema validation: ${(error as Error).message}`,
        eventKey,
        market
 ...
 */
export function logCrossProviderStats(
  quotesExtracted: number,
  arbsFound: number,
  durationMs: number
): void {
  logInfo('cross-provider.stats', {
    context: 'service:crossProviderCalculator',
    operation: 'findCrossProviderArbitrages',
    providerId: undefined,
    correlationId: undefined,

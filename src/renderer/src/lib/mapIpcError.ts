...
import type { ErrorCategory, IpcError } from '../../../../shared/errors'

/** Display type for error rendering */
export type ErrorDisplayType = 'inline' | 'banner' | 'errorBar'
...
export interface MappedError {
  ...
  guidance?: string
  /** Optional action text for a CTA button */
  actionText?: string
  /** Original error for debugging */
  originalError: IpcError
}
 ...
 */
export function mapIpcError(error: IpcError): MappedError {
 ...
 */
export function categoryToDisplayType(category: ErrorCategory): ErrorDisplayType {
  switch (category) {
    case 'UserError':
      return 'inline'
    case 'ProviderError':
...
function getUserErrorMessage(code: string, fallback: string): string {
  switch (code) {
...
function getUserErrorGuidance(code: string): string | undefined {
  switch (code) {
    case 'MISSING_API_KEY':
...
function getUserErrorAction(code: string): string | undefined {
  switch (code) {
    case 'MISSING_API_KEY':
    case 'INVALID_API_KEY':
...
function getProviderErrorMessage(code: string, fallback: string): string {
  switch (code) {
...
function getProviderErrorGuidance(code: string): string | undefined {
  switch (code) {
    case 'PROVIDER_TIMEOUT':
...
function getProviderErrorAction(code: string): string | undefined {
  switch (code) {
    case 'PROVIDER_TIMEOUT':
    case 'PROVIDER_UNAVAILABLE':
      return 'Retry Now'
...
function getSystemErrorMessage(code: string, fallback: string): string {
  switch (code) {
 ...
 */
export function isIpcErrorShape(value: unknown): value is { ok: false; error: IpcError } {
 ...
 */
export function extractIpcError(error: unknown): IpcError | null {
  ...
  if (typeof error === 'object' && error !== null) {
    ...
    if (obj.data && isIpcErrorShape(obj.data)) {
      return (obj.data as { error: IpcError }).error
    }
    ...
    if (
      ...
      return obj as unknown as IpcError
    }
  }
